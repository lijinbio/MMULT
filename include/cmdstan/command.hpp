...
namespace cmdstan {
  ...
  std::shared_ptr<stan::io::var_context> get_var_context(const std::string file) {
	...
	std::shared_ptr<stan::io::var_context> get_var_context(std::istream& stream) {
		stan::io::dump var_context(stream);
		std::shared_ptr<stan::io::var_context> result = std::make_shared<stan::io::dump>(var_context);
		return result;
	}
  ...
  static int hmc_fixed_cols = 7; // hmc sampler outputs columns __lp + 6
	...
	int command(int argc, const char* argv[], std::istream& sin, std::ostream& sout) {
		...
		// turn off info @20200204
		std::ofstream nullout("/dev/null");
		stan::callbacks::stream_writer info(nullout);
		stan::callbacks::stream_writer err(std::cout);
		stan::callbacks::stream_logger logger(std::cout, nullout, std::cout, std::cerr, std::cerr);
		...
		stan::callbacks::stream_writer sample_writer(sout, "# "); // to save the output to ostringstream instead of output file @20200204
		...
		std::shared_ptr<stan::io::var_context> var_context = get_var_context(sin);
		...
		// output_stream.close();
		// std::cout << sout.str() << std::endl; // stringstream to replace output_stream @20200204
